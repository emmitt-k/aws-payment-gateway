...
This project uses `golang-migrate` for database schema management. Migration files are stored in the `migrations/` directory.
...
The project provides simple Makefile commands for database migrations:
...
```bash
# Deploy (automatically migrates first)
...
# Rollback
./scripts/migrate.sh dev down
./scripts/migrate.sh prod down

# Force to specific version
./scripts/migrate.sh dev force 5
```
...
Files are executed in numerical order. See `migrations/README.md` for detailed instructions.

### AWS Database Connection
The migration script automatically reads database connection information from Terraform outputs and environment variables:
- Database host and port from Terraform outputs
- Database name and user from Terraform outputs  
- Database password from `terraform/dev.tfvars` or `terraform/prod.tfvars`
